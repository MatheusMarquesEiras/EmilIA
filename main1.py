...
class AuxServer:
    def __init__(self, _url: str, _model, sys: str = ''):
        self.client = Client(host=_url) 
        self.model = _model
        self.filename = "screenshot.jpg"

        # Dicionário mapeando nome -> função
        self.tools_list = {
        ...
        # Lista de objetos (funções) para passar ao Ollama
        self.tools = [
            self.pass_turn,
            self.add_numbers,
    ...
    def web_search(self, query: str) -> list[str]:
        """Realiza uma pesquisa na internet sobre um tópico."""
        try:
    ...
    def take_screenshot(self) -> str:
        """Tira um print/screenshot da tela do computador e analisa o que está nela."""
        try:
            with mss.mss() as sct:
...
class MainServer:
    ...
    # --- MÉTODO CORRIGIDO: Agora lê as Docstrings ---
    def analyze_intent(self, user_text: str) -> bool:
        """
        Analisa se deve usar ferramentas lendo a documentação real das funções.
        """
        tools_dict = self.aux_server.get_tool_list()
        
        # Cria uma lista formatada: "- nome_funcao: o que ela faz"
        menu_ferramentas = []
        for name, func in tools_dict.items():
            if name == 'pass_turn': continue # Ignora pass_turn no menu
            
            # Pega a docstring da função ou usa um texto padrão se não tiver
            descricao = func.__doc__ if func.__doc__ else "Sem descrição disponível."
            # Limpa espaços extras da docstring para economizar tokens
            descricao = " ".join(descricao.split())
            
            menu_ferramentas.append(f"- {name}: {descricao}")
        
        # Junta tudo em uma string gigante
        tools_description_str = "\n".join(menu_ferramentas)

        prompt_controller = f"""
        Analise a mensagem do usuário: "{user_text}"
        
        Abaixo estão as ferramentas disponíveis e suas descrições:
        {tools_description_str}
        ...
        Regras:
        1. Responda 'true' SOMENTE se o usuário pedir algo que corresponde explicitamente à descrição de uma ferramenta acima.
        2. Responda 'false' para conversas genéricas, saudações ou perguntas que não usem as ferramentas listadas.
        """
        
        try:
            response = self.client.chat(
                model=self.model,
                messages=[{'role': 'user', 'content': prompt_controller}],
                format='json',
                options={'temperature': 0} 
            )
            
            data = json.loads(response['message']['content'])
            requires = data.get('requires_tool', False)
            return requires
            
        except Exception as e:
            log_error(f"Erro no analisador de intenção: {e}")
            return False 

    def stream(self, _messages_formated: list, tools_to_use: Optional[list] = None):
    ...
    def generate_test(self):
        user_message = UserMessage(message='[Professor] mensagem de teste do sistema.')
        ...
        try:
            ...
            for part in self.stream(messages_to_send, tools_to_use=None):
                log_info(part)
    ...
    def generate(self, message):
        ...
        try:
            # 1. ROTEAMENTO INTELIGENTE (LÊ AS DOCSTRINGS)
            requires_tools = self.analyze_intent(message)
            
            current_tools = None
            if requires_tools:
                log_info("Modo: 🛠️  COM FERRAMENTAS")
                current_tools = self.aux_server.get_tools()
            else:
                log_info("Modo: 💬 CONVERSA PURA")
                current_tools = None

            # 2. GERAÇÃO
            stream_generator = self.stream(self.messages, tools_to_use=current_tools)
            ...
            if not full_text:
                if used_pass_turn:
                    log_info("IA decidiu passar a vez.")
                    return None 
                
                log_warning("IA retornou resposta vazia.")
                return "Não entendi."
